...
class ADSMapper():
    ...
    def write(self, adsAdr, value):
        ...
        err = adsSyncWriteReq(
            adsAdr, indexgroup, self.plcAdr,
            self.currentValue, self.plcDataType)
        if err == 0:
            return
        raise Exception("error writing on address %i. error number %i" %
                        (self.plcAdr, err))
    ...
    def read(self, adsAdr):
        ...
        (err, value) = adsSyncReadReq(
            adsAdr, indexgroup, self.plcAdr, self.plcDataType)

        if err:
            raise Exception(
                "error reading from address %i (%s). error number %i" %
                (self.plcAdr, self.name, err))
    ...
    def mapAdsToGui(self, guiObject, value):
        """
        Display the value on the connected gui object, this function
        should be overriden, by default the value is printed on the console.
