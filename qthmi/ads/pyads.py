...
def adsSyncWriteControlReq(adr, adsState, deviceState, data, plcDataType):
    ...
    Note:
        ...
        Despite changing the ADS-state and the machine-state it is
        possible to send additional data to the ADS-server.
        For current ADS-devices additional data is not progressed.
        Every ADS-device is able to communicate its current state
        to other devices. There is a difference between the device-state
        and the state of the ADS-interface (AdsState).
        The possible states of an ADS-interface are defined in the
        ADS-specification.
    ...
    errCode = adsSyncWriteControlReqFct(
        pAddr, nAdsState, nDeviceState, nLength, pData)
    return errCode
...
def adsSyncWriteReq(adr, indexGroup, indexOffset, value, plcDataType):
    ...
    :param AmsAddr adr: local or remote AmsAddr
    :param int indexGroup: PLC storage area, according to the
        INDEXGROUP constants
    :param int indexOffset: PLC storage address
    :param value: value to write to the storage address of the PLC
    :param plcDataType: type of the data given to the PLC,
...
def adsSyncReadReq(adr, indexGroup, indexOffset, plcDataType):
    ...
    :param int plcDataType: type of the data given to the PLC,
        according to PLCTYPE constants
    :rtype: (int, PLCTYPE)
    :return: (errCode, value): ``errCode`` error-state of
        the function, ``value``
